...
class SetCriterion(nn.Module):
    ...
    # def loss_point(self, outputs, targets, indices, num_points, theta=0.2, **kwargs):
    #     """位置损失使用LFF机制"""
    #     assert 'pred_points' in outputs
    #     idx = self._get_src_permutation_idx(indices)
    ...
    #     # 计算每个预测点与目标点之间的L1损失
    #     loss_point = F.l1_loss(src_points, target_points, reduction='none')  # (nt, 2) 每个预测点的损失
        
    #     # 计算每个预测点的总损失（这里假设按2维的L1距离计算）
    #     point_loss = loss_point.sum(dim=1)  # (nt,) 每个预测点的损失总和（针对2维坐标）
        
    #     # 计算自适应的 theta 值（根据当前图像的损失分布）
    #     # theta = self.get_dynamic_theta(point_loss)  # 获取动态的 theta

    #     # 对损失进行排序
    #     sorted_loss, sorted_indices = torch.sort(point_loss)
        
    #     # 计算保留损失的数量（按比例theta）
    #     num_losses = point_loss.numel()
    #     keep_losses = int(num_losses * (1 - theta))  # 保留最小的 (1 - theta) 部分的损失

    #     # 创建掩码，保留损失较小的点
    #     mask = torch.zeros_like(point_loss, dtype=torch.bool)
    #     mask[sorted_indices[:keep_losses]] = 1  # 保留最小的损失点

    #     # 使用掩码过滤掉较大的损失（即噪声点）
    #     filtered_loss_point = point_loss * mask.float()

    #     # 返回过滤后的损失（求和并平均）
    #     filtered_loss = filtered_loss_point.sum() / num_points  # 计算过滤后的总损失

    #     losses = {'loss_point': filtered_loss}
